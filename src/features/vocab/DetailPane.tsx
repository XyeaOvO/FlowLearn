...
import type { Word, Settings } from '@common/types'
...
type DetailPaneProps = {
  ...
  onSave: (updated: Word) => Promise<void> | void
  settings: Settings | null
}

export default function DetailPane({ word, speak, onDelete, onSave, settings }: DetailPaneProps) {
  ...
  const updateDraft = useCallback(<K extends keyof Word>(key: K, value: Word[K]) => {
  ...
  const analysisHtml = useMemo(() => {
    ...
    return DOMPurify.sanitize(marked.parse(word.analysis) as string)
  }, [word?.analysis])
  ...
  if (lastReviewedAt && stability && stability > 0) {
    ...
    if (!Number.isFinite(retrievability) || retrievability < 0) retrievability = 0
      ...
      <div className="detail-content">
        <div className="detail-header">
          <div className="detail-title">
            {word.term}
            <button className="tts-button-detail" onClick={() => speak(word.term)} aria-label={t('actions.pronounce') ?? 'speak'}>
              <VolumeIcon size={20} />
            </button>
            <span
              className={`badge ${
                word.reviewStatus === 'new' ? 'badge-info' : word.reviewStatus === 'learning' ? 'badge-warning' : 'badge-success'
              }`}
              ...
              <div className="detail-section-content">
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span className="badge badge-neutral">{`${retrievabilityPercentage}%`}</span>
                    </div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <div className="meta-label">{t('detail.repetitions')}</div>
            ...
            <div className="detail-section">
              ...
              <div className="detail-section-content" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div className="form-group">
                  ...
                  <input className="input" value={draft.phonetic} onChange={e => updateDraft('phonetic', e.target.value)} />
                </div>
                <div className="form-group">
                  <label className="form-label">领域</label>
                  <input className="input" value={draft.domain || ''} onChange={e => updateDraft('domain', e.target.value)} />
                </div>
                <div className="form-group">
                  <label className="form-label">释义</label>
                  <textarea className="textarea" style={{ height: 100 }} value={draft.definition} onChange={e => updateDraft('definition', e.target.value)} />
                </div>
                <div className="form-group">
                  <label className="form-label">例句</label>
                  <textarea className="textarea" style={{ height: 120 }} value={draft.example} onChange={e => updateDraft('example', e.target.value)} />
                </div>
                <div className="form-group">
                  ...
                  <input
                    className="input"
                    type="date"
                    value={toDateInputValue(draft.reviewDueDate)}
                    onChange={e => updateDraft('reviewDueDate', e.target.value ? new Date(`${e.target.value}T00:00:00`).getTime() : null)}
                  />
        ...
        <div className="toolbar">
          {!isEditing && (
            <button className="btn" onClick={() => { setIsEditing(true); setDraft({ ...word }) }}>
              <EditIcon size={16} /> {t('actions.edit')}
            </button>
          )}
          {isEditing && (
            <>
              <button className="btn" onClick={() => { setIsEditing(false); setDraft({ ...word }) }}>{t('actions.cancel')}</button>
              <button className="btn btn-primary" onClick={handleSave}>{t('actions.save')}</button>
            </>
          )}
          <button className="btn btn-danger" onClick={() => void onDelete()}>{t('actions.deleteWord')}</button>
        </div>

        {!isEditing && word.analysis && (
          <div className="analysis-card">
            <div className="analysis-header">
              <div className="analysis-title">{t('detail.analysis')}</div>
              <div className="toolbar">
                <button className="btn btn-ghost" onClick={() => void navigator.clipboard.writeText(word.analysis ?? '')}>
                  {t('actions.copy')}
                </button>
                <button
